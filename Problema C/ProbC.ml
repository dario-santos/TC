...
(* 
  ...
  Output: Verificar se alguma secção da palavra valida a expressão regular.


  Notas: Acredito que esta solução necessite de automatos.

  1 - Converter a expressão regular para um automato finito não deterministico.
  2 - Correr a palavra no automato (Com janela).
      * Mal for validade termina;
      * Se corrermos toda a execução então não foi aceite. 
*)
